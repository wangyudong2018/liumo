...
import com.yiyun.web.base.service.GeneratorService;
import com.yiyun.web.common.utils.GenUtils;
...
public class GeneratorController {
	...
	List<Map<String, Object>> list() {
		final List<Map<String, Object>> list1 = generatorService.list();
	...
	public void code(HttpServletRequest request, HttpServletResponse response,
		...
		response.reset();
		response.setHeader("Content-Disposition", "attachment; filename=\"template-" + tableName + ".zip\"");
	...
	public void batchCode(HttpServletRequest request, HttpServletResponse response, String tables) throws IOException {
		...
		byte[] data = generatorService.generatorCode(tableNames);
		response.reset();
        response.setHeader("Content-Disposition", "attachment; filename=\"template-batch.zip\"");
        response.addHeader("Content-Length", "" + data.length);
		response.setContentType("application/octet-stream; charset=UTF-8");
